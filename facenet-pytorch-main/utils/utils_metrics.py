...
def calculate_val(thresholds, distances, labels, far_target=1e-3, nrof_folds=10):
    ...
    for fold_idx, (train_set, test_set) in enumerate(k_fold.split(indices)):
        ...
        if np.max(far_train)>=far_target:
            ...
            unique_far_train = far_train[unique_indices]
            unique_thresholds = thresholds[unique_indices]

            sort_indices = np.argsort(unique_far_train)
            unique_far_train = unique_far_train[sort_indices]
            unique_thresholds = unique_thresholds[sort_indices]

            if len(unique_far_train) > 1:
                f = interpolate.interp1d(unique_far_train, unique_thresholds, kind='slinear')
                threshold = f(far_target)
            else:
                threshold = unique_thresholds[0] if len(unique_thresholds) > 0 else 0.0
        else:
            threshold = 0.0
